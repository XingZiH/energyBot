...
const templateDemoAuthCodePrefixes = ['default:dashboard', 'default:page-demo', 'default:feat', 'default:comp', 'default:level', 'blank:other-login'] as const;
const templateDemoAuthCodes = new Set(['default:about', 'blank:empty-page']);

function isTemplateDemoAuthCode(code: string): boolean {
  return templateDemoAuthCodes.has(code) || templateDemoAuthCodePrefixes.some(prefix => code === prefix || code.startsWith(`${prefix}:`));
}
...
export const user = [
  ...
  http.get('/site/api/user/auth-code/:id', ({ params }) => {
    ...
    const codes = Number(params['id']) === 1 ? adminAuthCodes.filter(code => !isTemplateDemoAuthCode(code)) : [];
