...
- 全部 23 个任务中，20 个 commit 已落 main（HEAD `c2bea33f`）
- 生产部署待 SSH 恢复后按 `docs/deployment/b1-launch.md` 手工执行
- 任务清单对应 git commit SHA，见下方任务 0–22 标题旁的 `[x] [SHA]` 标记
...
## 任务 0: [x] [ba9bbedb]：依赖安装 + ScheduleModule 引入
...
## 任务 1: [x] [d09d1dca]：agents 表 + schema.ts + migration + rollback
...
## 任务 2: [x] [387a5e05 + 4961a080(5.5 runPrecheckCore)]：LicenseService.verifyPrecheckForHandshake
...
## 任务 3: [x] [5defc962]：HMAC fixture 文件
...
## 任务 5: [x] [9ebbd543]：AgentRegistry（内存状态机）
...
## 任务 6: [x] [634bbe75]：AgentService（DB 层 + 去抖心跳）
...
## 任务 7: [x] [482ca8be + 25f329b9]：AgentGateway
...
## 任务 8: [x] [155bbcf9]：AgentOfflineScheduler
...
## 任务 9: [x] [36c4caa5]：AgentController GET /agent/my-bot
...
## 任务 10: [x] [d750f505]：AgentModule 装配
...
## 任务 11-16: [x] [见子任务]：Go agent 端

Go 端每个包都遵循 TDD 模式。结构已在"文件结构"列出。关键实现点：

### 11: [x] [78918b81]：`internal/auth/hmac.go` + `hmac_test.go`
...
### 12: [x] [87fc9e3d]：`internal/jsonrpc/codec.go`

- [ ] 定义 `Request`, `Response`, `ErrorObj` 结构体；Marshal/Unmarshal 测试
- [ ] Commit

### 13: [x] [3ab824f6]：`internal/host/host.go`
...
### 14: [x] [12590fc1]：`internal/client/client.go`
...
## 任务 18: [x] [5f14f621]：Angular「我的 Bot」页
...
## 任务 19: [x] [94194799]：install.sh 新增 install_agent()
...
## 任务 20: [x] [94194799]：nginx /agent location + docker-compose
